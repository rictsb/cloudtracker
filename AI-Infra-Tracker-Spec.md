...
11. **GPU ramp** — the quarterly capacity/fleet/revenue path for GPU-cloud owners (per-company `ramp` object; IREN first, CRWV/NBIS as researched). Three layers on one time axis, generation-colored (Hopper/Blackwell/Rubin-class/next-gen): *Concrete* — capacity tranches from energization to first revenue (thickness = gross MW, solid = signed today, hatched = uncontracted, ◆ = first revenue); *Silicon* — revenue-generating GPUs stacked by generation with the signed-book line; *Money* — quarterly revenue bars by generation + residual mining vs the Bloomberg consensus line; plus the quarterly table (gross MW cum, critical-IT MW active, GPUs added/cum, signed-today % vs modeled contracted %, blend $/GPU-hr, revenue, consensus, Δ). RESEARCH OVERLAY: filings-anchored near-term (company-disclosed programs), modeled cadence beyond; rates from the contract print tape; NEVER a valuation input — the engine never reads `ramp`. Every model carries `asOf` + `basis`; refresh cadence = each earnings/8-K event on a covered name.
