...
1. **Comparison dashboard** — all names ranked by upside; each row carries a **value gauge** (bar = our target value split contracted-floor / expected / legacy; dark line = market price; shaded gap = upside or overvalued) and a collapsed **valuation narrative** toggle. Each row also carries the **execution floor** — the target recomputed with the unsigned/expected slice at zero (signed contracts + legacy only, same claims/discount/dilution) — shown as a dashed tick on the bar and a floor-vs-price line: the visible measure of how much of each target is executed fact vs awarded future.
