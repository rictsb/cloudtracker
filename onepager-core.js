...
(function (root, factory) {
  ...
  function waterfall(L, CAPQ, F, ARRC, o) {
    ...
    else for (let it = 0; it < 40; it++) { S = (ev - last.nd - liab + NC) / (sh30 + convSh) * 1000; convSh = SERIES.reduce((a, s) => a + s[0] / s[1] * 1000 * (S > s[1] ? 1 : 0) * (s[2] ? (1 - Math.max(0, Math.min(s[2], S) - s[1]) / S) : 1), 0); }   // a series converts only when the horizon price clears its conversion price; a capped call (s[2]) nets the shares between strike and cap
  ...
  function steadyValue(p, term, Wd) {
    ...
    for (let y = 1; y <= H; y++) {
      ...
      const gpuY = p.gpu * Math.pow(1 + (p.costG || 0), Math.min(y, p.gY || 0));   // costG: the refresh buys the then-current generation at the then-current cost per MW
      const eb = rate * p.M, dep = gpuY * p.swap / p.life + sh, tax = Math.max(0, eb - dep) * p.tax;
      let fcf = eb - sh - tax - (p.fail || 0) * gpuY; if (y % p.life === 0) fcf -= gpuY * p.swap;
