...
`quotes.js` shares Finnhub equity quotes across summary, asset and research pages through `CloudModel.setMarks`. It checks at load/return when due, every 30 minutes while open, and via manual refresh. A browser cache retains the last usable observation. Every company has separate provider quote time, retrieval time and source; failed names retain their own timestamps, unknown provider times stay unknown, and cached/saved fallback states are explicit. Never give untouched names a new cycle timestamp. Quotes update market references, upside, chart markers and price-dependent ratios; the research funding sensitivity uses the displayed reference price. Canonical dated research inputs and base values are immutable under quote refresh. `verify-quotes.cjs` and `verify-market-research.cjs` are pre-push/CI gates alongside existing checks.
