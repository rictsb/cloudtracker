...
- `quotes.js` shares per-company market prices and provider clocks across the application; load the shared `market-prices.json` snapshot on entry and every 30 minutes, with direct-provider fallback. `quote-snapshot.cjs` and dispatch-only `refresh-quotes.yml` publish it under the scheduled GPT task; the server uses `FINNHUB_TOKEN`, never the browser credential file. Never stamp all companies with the latest fetch time. `research-data.js` supplies immutable research payloads and preferred main-comparison values for IREN/CRWV/NBIS; other rows retain the asset model. Missing research values stay unavailable, without asset-value substitution. Run `node verify-quotes.cjs`, `node verify-market-research.cjs` and `node verify-quote-snapshot.cjs` with the other gates.
