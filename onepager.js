...
const ROOT = __dirname, d = JSON.parse(fs.readFileSync(path.join(ROOT, 'data.json'), 'utf8'));
const c = d.companies.find(x => x.tk === tk); if (!c) { console.error('no such company: ' + tk); process.exit(1); }
const pg = c.page, R = c.ramp; if (!pg || !R) { console.error(tk + ' needs both a page block and a ramp block in data.json'); process.exit(1); }
...
const P = { tk, name: c.name, asOf: pg.asOf, kicker: pg.kicker, title: pg.title, sub: pg.sub, px: pg.px, evArr: pg.evArr, arrLabel: pg.arrLabel, capacity: pg.capacity, gantt, Q, A, L, CAPQ, ARRC,
  fund: pg.fund, bridge: pg.bridge, steady: pg.steady, finance: F, rr, year, gpuMax: Math.ceil(Q[Q.length - 1][6] / 250000) * 250000, arrMax: Math.ceil(A[A.length - 1][1] / 15) * 15, hero: pg.hero };
