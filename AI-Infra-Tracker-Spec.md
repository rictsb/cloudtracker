...
**Investability tier carries re-rating — a third axis, distinct from provenance and contracted %.** Provenance is *will it get built*, contracted % is *will it get leased*, and the tier is *what the market pays per dollar of stabilized income* as a name becomes more investable. Each company carries a tier (Speculative / Emerging / Proven / IG-REIT) from a fixed table that sets its cap rate (landlords, via a spread on the dial) or compute-value multiple (owners, via a factor). A miner-pivot with no effective leases sits at Emerging — not junk, not yet REIT. Re-rating is modeled by moving a name up tiers as it earns investability, and is baked into the headline target (not a separate band). The global cap-rate / multiple dials are the stabilized anchor the tiers adjust from.
