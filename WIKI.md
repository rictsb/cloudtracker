...
## Dashboard gauge
**Execution floor (dashed pine tick + "floor" line under each target):** what the equity is worth if NOTHING more is ever signed — executed contracts + legacy only, net of the same claims/discount/dilution as the headline target (`floorTarget`, computed in the shared engine). Floor-vs-price answers "what am I paying for execution?": names ABOVE their floor (green) are cheap on signed paper alone; the gap between floor and target is the bet on future signings that leaseUp=1.0 awards in full. Born of the owner's speculation thought-exercise 8/6/26 — visibility first, thesis unchanged.
