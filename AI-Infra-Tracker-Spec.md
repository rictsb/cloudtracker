...
- **Owner (cloud):** `value/MW = effective rate × margin% × multiple` — two slices, split by the compute-contract registry:
  - **Signed slice**: earns the company's `signedRate` — the $-weighted rate of its actual take-or-pay book (`contracts[]`: dollars + term are the facts; MW is usually inference, so the rate binds at COMPANY level, basis-noted and checks-reconciled against the registry). An optional `signedRegionFactor` multiplies only signed income; absent, the existing site-region multiplier applies. Unsigned/re-sign pricing retains its own site geography and realization assumptions. The signed-rate basis must distinguish contractual amounts from adopted expected-value estimates. Lock = `min(term ÷ capitalization-years (≈ the multiple), 1) × contracted%` — a 5-yr book on a ~7-yr capitalization leaves ~30% of every contracted MW at the RE-SIGNING.
...
- **Valuation inputs** (move the target): `model`, `tier`, `sites[]` (`{n, mw, owned, region, yr, mo, prov}`), `contracts[]` (owner compute-contract registry: {id, counterparty, totalRevM, termYrs, signed, gen: hopper/blackwell/vera-rubin/mixed/tpu, inferredMW, ratePerMWyr?, effective, source}) + company `signedRate` (+basis), optional owner-only `signedRegionFactor` (finite >0 and ≤2; absent uses the site region rate factor; 1 preserves an adopted local contract price without a second geography adjustment), and `genAccess` (+basis if ≠1), `leases[]` (the registry: one record per signed rate book — {id, counterparty, mw, termYrs, noiPerMWyr (term-average), kind, effective, signed, source}; sites link via `leaseId`, JV slices carry `physMW`), `contractedPct` (owners; derived from the registry for landlords), `termYrs` (owner lock), `netDebt`, `committedDebt`, `seniorClaims`, `shares`, `legacyEV` (non-BTC residual), `btc`/`eth` (counts, if any), `stake` ({tk,pct}, holdcos), `plannedRaise`, `equityDiscount` (default 0).
...
## 6. The screens (fourteen)

> **2026-09-15 presentation amendment (owner-approved redesign cutover).** The screens below now render inside the redesigned single-page app (Compare / Infrastructure / Research / Operations; `index.html` + `app.js` + per-screen view modules). Every screen's information contract in this section is unchanged and remains binding. Research one-pagers (§6d–6e) and the comparison page (§6f) are client-rendered as continuous graphical documents from generated `<tk>-data.json` / `compare-data.json` (`node export-research.js`), replacing generated HTML pages; the math stays in `onepager-core.js`, shared and never forked. Legacy `#`-links migrate via a client-side shim; path routing rules live in `render.yaml`.
...
8. **Outlook — the catalyst board** — the forward book, refreshed weekly by the Monday sweep and stored as a top-level `outlook` object (present-tense, overwritten; **judgement only — NEVER a valuation input, and NEVER a sizing input to the paper portfolio (§6b)**; owner ruling 2026-09-22: display-only, revisit with evidence after a quarter). Three sub-views. *Catalyst board* (default): every plausible, dated, material event for every name inside ~6 months, of ANY kind — `kind` ∈ lease / contract / power (PPA, interconnection, turbines) / energization / financing (raises, covenants, project bonds) / jv / site / regulatory / earnings / other — one row each in `outlook.catalysts[]`: `{tk, id, kind, title, prob 0-100, window, by (ISO hard date or null), priced 0-100 (how much the market already discounts it), impact, drivers[], sources[]}`. `impact` describes the event as a change to the tracker's own inputs (`catalyst-core.js` ops: `prov` upgrade, `lease` on a site, `energize` date move, new `site`, equity `raise`, `debt` change, `tier` re-rating, or `multi`) and the **shared engine prices it live** — Δtarget and Δfloor per share at the current dials and marks, never stored. Rank = `prob × max(|Δtarget|, |Δfloor|) ÷ price × (1 − priced)` — a probable, soon, unpriced, target-moving event ranks first regardless of its kind; a row with `impact: null` (earnings, most regulatory) shows but scores zero. Negative catalysts (a likely raise, a covenant election) carry negative Δ and rank on magnitude. Filters: company, kind, hard-dated only. Rows expand to the evidence trail with source tags per the §9 registry. *Next leases* (`outlook.leases[]`): the legacy lease-only view — probability of a binding lease/compute contract, expected MW range, window, candidate tenant/site, build-speed score, evidence. *Earnings setup* (`outlook.earnings[]`): per reporting date (confirmed vs estimated), a −5…+5 surprise score RELATIVE TO WHAT IS PRICED, the key metric, consensus anchor, drivers, sentiment. Checks: ticker, kind, ranges, source registry, and every `impact` must apply and price without error (the board never renders a broken op).
...
12. **News** — the curated sources, summarised: one item per video from the registered commentary feeds (§9), newest first, with date, source, type (interview / earnings / news / members), the tickers it covers (coverage universe only), our own summary, and the signal claims the extraction tagged (scoop / call / emphasised, each with who said it and a link to the moment in the video). Filter by name and by signal-only. Read from a top-level `news.json` — a rolling 120-day window overwritten daily by the DGX Spark pipeline (§6g). Summaries are ours; transcripts never leave the Spark. Display-only; never a valuation input.
13. **Approvals** — the proposal queue rendered: every pending change the Spark pipeline has proposed against `data.json` (a site's energization date or MW that disagrees with a company-sourced statement; a dated development for a name's `log[]`; a catalyst), each with the current value, the proposed value and the dated quotes and video links behind it, plus Yes / No. **Yes** commits the decision to `proposals.json` (the browser writes to GitHub with a repo-scoped token the owner keeps only on their own devices); the `apply-proposals` Action then writes the fact into `data.json`, adds the CHANGELOG line, and the site redeploys. **No** records the decision so the proposal is never raised again. Recent decisions and their applied / error status are listed below the queue. This is the §9 "writes are gated" boundary made clickable: nothing enters the model without a decision.

### 6j. Shared market references and preferred summary values

IREN's owner-selected default includes a 0.5× house premium above the DCF-derived EV/revenue multiple, stored as `page.valuationMultiplePremium` and applied once by canonical assembly. The report, summary and research comparison use this default. The underlying DCF multiple is retained and disclosed; the no-premium case is available as the −0.5× sensitivity. Other sensitivities run around the current house default. CRWV and NBIS retain their DCF-derived defaults.

The Power section of each research report shows cumulative year-end capacity beneath the campus and tranche timeline. Q4 rows from the canonical research ledger supply energised IT MW (primary), gross MW and earning IT MW for 2026 through the report horizon. These are closing totals including the opening fleet, not sums of the displayed tranches or annual additions. Company-specific active/connected terminology is retained; future totals remain modeled.

The main comparison opens with GPU operators selected. Its header offers Model assumptions without a separate research shortcut; the model-value column labels research values “Research”. It uses the canonical research DCF values for IREN, CRWV and NBIS, obtained through `research-data.js` from the same generated payloads and `OnePager.waterfall` as their reports. Other names retain the asset model. Sorting, displayed upside, the median and CSV exports use those preferred values and the displayed market references. Research-valued rows omit the asset-only signed scenario and link to their research report. Missing research data produces an explicit unavailable value and retry, never an asset-model substitution. `/company/<TK>` remains the separate asset model with a link to the preferred report. Asset dials do not change research base values. This presentation policy does not change the paper allocator or historical ledger.

`quote-snapshot.cjs` retrieves Finnhub equity quotes (server environment credential) and Coinbase spot marks into the present-tense `market-prices.json`; it never changes research inputs or the portfolio ledger. The `refresh-quotes.yml` dispatch-only job publishes that snapshot with serialized, bounded concurrent-push recovery. The owner's GPT task dispatches and verifies it every 30 minutes. A desktop GPT task requires its computer and app to remain running; there is no hidden GitHub cron fallback.

`quotes.js` shares those references across summary, asset and research pages through `CloudModel.setMarks`. It checks the shared snapshot on entry, every 30 minutes while open, when overdue on return and via manual refresh. Provider fallback preserves operation if the shared snapshot is missing, stale or incomplete; a browser cache retains last-good observations. Every company has separate provider quote time, retrieval time and source. Failed names retain their own timestamps; unknown provider times stay unknown; cached/saved fallback states are explicit. Snapshot `checkedAt` is job activity, never a replacement for quote time. Quotes update market references, upside, chart markers and price-dependent ratios; the research funding sensitivity uses the displayed reference price. Canonical dated research inputs and base values are immutable under quote refresh. `verify-quotes.cjs`, `verify-market-research.cjs` and `verify-quote-snapshot.cjs` are pre-push/CI gates alongside existing checks.

### 6i. Comparable research pricing and contract cohorts

The IREN, CRWV and NBIS research reports share `researchPricing` in `data.json` and the canonical `ramp-core.js` calculation. It feeds their separate research valuations; it does not change `engine.js` or the paper allocator. Report JSON is generated by `export-research.js`, never edited manually. The client reports retain the continuous graphical layout and dated market-price reference.

- **Separate commercial states.** Existing commitments, same-hardware renewals, new business and spot each have their own revenue series. A renewal is a new assumption after an exclusive expiry quarter, not signed revenue indefinitely. Only original-term revenue on disclosed customer allocations is displayed as mapped to confirmed coverage. Missing mapping is not evidence that the company has no backlog.
- **Allocation uncertainty.** Physical tranches may be allocated to existing customer business only with a separate evidence status. Unmapped tranches energizing from the explicit 2027Q1 cutoff use new-business prices; the 2026 in-flight book remains inferred. This cutoff is a house convention. A legacy-book sensitivity restores the earlier inferred future allocations. Contracted power alone cannot prove compute-contract prices or terms. Original `signed` fields remain for pre-policy historical calibration; documented `contract.signedShare` overrides apply to forward pricing only.
- **Common market assumptions.** New business locks an all-in $m/IT-MW/year curve at service commencement. The base curve is $18m in 2026–27, $19m in 2028, $20m in 2029 and $21m in 2030–31. It is a house assumption, including the flat 2031 extension. Three-year new contracts and renewals, 100% continuation at the same hardware/rate, zero software premium, uncalibrated tenor spreads and the shared spot curve are explicit. No double counting of software attach. Future price, renewal, retention, tenor, delivery and combined scenarios are illustrative, not probabilities; released capacity uses the explicit spot assumption.
- **Comparable denominators.** Earned contracted revenue annualized divided by contracted earning IT MW is a contract yield. All earned revenue annualized divided by all earning IT MW is total yield. Neither uses gross or merely energized MW. Modeled full-capacity ARR is separate and must not be described as signed backlog or an earnings run-rate during commissioning.
- **Dynamic economics.** Normalized per-MW revenue and contract/spot revenue mix regenerate the terminal DCF multiple; margin, shell costs, GPU costs and financing remain company-specific. The five-year scenario also changes the normalized terminal regime. The long-run refresh reserve is distinct from physical retirement at contract expiry. Forecast and normalized margins are labeled separately.
- **Financing consistency.** Future prepayment receipts and lending capacity remain explicit financing assumptions, not secured funding. Modeled credits follow the service period and assumed term; mixed tranches allocate credits by contractual IT MW. Fixed opening-balance schedules and special receipts remain distinct. Post-horizon credits are included in the liability at PV; an unscheduled residual is charged at face value. Delivery delays retain the committed purchase-capex calendar. The deliberately uncredited sensitivity waives all credits and liabilities and is not an investable base.
- **Checks.** Pricing checks validate dates, sources, allocation overrides, expiry logic and quarterly revenue reconciliation. Evidence gaps are grouped by company rather than repeated for every quarter. `verify-pricing.cjs`, `verify-pricing-checks.cjs` and `verify-pricing-finance.cjs` cover state transitions, parity, provenance, sensitivities and the financing calendar, alongside the existing gates. Historical fit is preserved separately and does not validate future pricing.

### 6j. Assumption Review (owner-authorized 2026-09-16)

`/research/assumptions` is the economic-review inbox under Operations. It uses the existing `/research/*` deployment rewrite. It covers every company and distinguishes research DCFs, asset models and holdcos. The first three families are capacity-to-revenue, equipment economics, and funding-to-common-equity. Hard contradictions, economic questions, and missing independent evidence are separate. Missing data never passes; peer comparisons require comparable scope, dates, units, ownership and business model. Quarterly revenue annualized by four is labeled as an annualized quarter, not a measured December exit rate.

- **Reviewer.** `assumption-core.cjs` evaluates current data using canonical calculations in engine/ramp/onepager modules. Sensitivities use those same models and are not independent proof of assumptions. Findings have stable rule/entity IDs and input-dependent fingerprints. Price/retention stress that increases modeled value must disclose why; no scenario is called downside merely from its label. Rules, thresholds, scenario descriptions and evidence limitations travel with results. No target, input or portfolio position is changed automatically.
- **Asset-model review.** Missing quarterly detail does not exempt a name from economic questions. The reviewer measures unsigned-value concentration and realization sensitivity, material planned issuance and issue-price risk, model-versus-reported shares, signed-lease cap-rate floor and residual-value dependence, company-derived signed rates receiving regional discounts, look-through ownership versus claims, and treasury asset/claim scope. These are explicit materiality screens and reconciliation questions, not proof of mispricing; no coverage quota forces a company to fail. Asset sensitivities use the canonical engine and retain existing provenance haircuts. Contracted asset value exceeding full undiscounted initial-term NOI is a residual-value attribution question, not an assertion that property residual value is zero. Source narratives are shown for reconciliation, never parsed into automatic numerical corrections.
- **Background state.** `assumption-review.cjs` writes the single present-tense derived `assumption-review.json`. It carries actual completion time, rule version, source/quote/model hashes, company coverage, findings and basis. Failure publishes an explicit error without a fresh-success timestamp. The background Action runs on relevant model/data changes, after quote/approval/portfolio workflows, and nightly at 03:17 UTC. `workflow_dispatch` allows a manual run. The run must re-read latest main and recompute after push conflicts. Its own publication does not recursively trigger it. The published snapshot and generated research are verified with bounded retries after deployment. A numerical run never stamps filings as externally reverified.
- **Review page.** `assumption-view.js` / `assumption-review.css` display material questions first, with all gaps in filters and the coverage matrix. Impacts are individual scenarios, never summed. The browser compares the reviewed data hash with deployed data before enabling triage; mismatches, missing data, job errors and overdue results are explicit. Review time, evidence-review time and quote times are separate. Read-only rendering works without credentials.
- **One queue.** Triage lives in `proposals.json.assumptionReviews`, keyed by finding ID with fingerprint, reason, status (`research` or `kept`), review date and update time. Keep decisions apply only to economic assumptions, require rationale and expire within 90 days; altered findings reopen. Review actions reuse the device's existing repo-scoped authorization. `?dryrun=1` previews without writes. A stale browser cannot overwrite a newer review decision.
- **Evidence research.** The scheduled analyst checks requested findings and material source changes, and performs a weekly broader sweep. It uses primary filings/IR evidence and existing curated-source ceilings. The heartbeat runs in this task; the machine and Codex app must be available. `assumption-research.cjs --input <review.json>` records only the findings actually investigated into `proposals.json.assumptionResearch`; it refuses changed fingerprints and requires sources for explained or proposal-ready outcomes. Records are current state, overwritten, not a new history. Unsupported comparisons remain unresolved. Ordinary unchanged results stay quiet.
- **Numerical proposals.** `assumption-proposals.cjs --input <proposal.json> [--check]` creates only pending `kind: assumption` proposals. Explicit allowed scalar paths, units, expected current values, evidence and rationale are required. Each proposal includes exact before/after values, all affected names, canonical revenue/funding/equity impacts and relevant source/model/quote identities. Shared changes are atomic bundles; arbitrary JSON writes, derived report edits and automatic assumption decisions are prohibited. The only creatable optional scalar is owner `signedRegionFactor`: its displayed current value must be the unique canonical multiplier across sites with signed income; an empty or mixed-region signed book is rejected. All other approved paths must already exist as finite numbers. Existing review IDs/decisions and Spark queue fields must round-trip unchanged.
- **Approval and application.** Existing Approvals displays numerical changes and impact previews, with an explicit refresh control to retrieve newly published proposals. Decisions bind the exact proposal reviewed. The worker checks freshness, recomputes impacts, rejects conflicts and new hard failures, and regenerates canonical research exports before committing. Approved, applied and failed states are distinct; deployment verification belongs to the completed review job and must not be inferred from approval alone. Validation runs before publication. Git commits and CHANGELOG remain the durable audit trail.
- **Validation.** `verify-assumptions.cjs`, `verify-assumption-runner.cjs`, `verify-assumption-approvals.cjs` and `verify-assumption-view.cjs` cover economic contradictions, coverage, independence limitations, hash/freshness checks, publication parity, safe numerical paths, atomic application, decision conflicts, rendering and read-only failures. Existing application/pricing/finance/quote checks continue to apply.
...
### 6f. The comparison page

`compare.html` sets the research pages side by side: one column per name, one row per gating factor (business model and power, cost of the shell, power price, silicon per MW, book life, contract length, take-or-pay and prepayments, customer mix, price per MW-yr by tenor, cost of IG-backed and non-IG GPU debt, corporate debt and converts, equity record, EBITDA margin, stated payback, capacity, next-generation silicon), each cell a short sourced value with a trend call (converging or structural); then what one megawatt keeps, the horizon-year picture, and the convergence read. It is generated by `node compare.js` from the `compare` block in `data.json` (the factor rows, the notes and the footer — present-tense, sourced) and from the same `page` and `ramp` blocks that generate each research page: every number is computed by `onepager.js assemble()`, the comparison page computes nothing of its own. It is a dated snapshot under §6d rules (regenerated by hand, one CHANGELOG line, not linked from the app’s navigation, display-only).

### 6g. The news + approvals pipeline (the Spark side)

The curated commentary feeds are processed on the owner's DGX Spark, outside the repo: videos are transcribed locally (whisper.cpp), summarised (Hermes) and broken into structured claims (Gemma: ticker, site, claim, number, who said it, recap / analysis / scoop / call, emphasis, horizon), all held in one SQLite database there. Once a day the pipeline writes two files into the repo with a write-scoped deploy key — `news.json` (screen 12) and `proposals.json` (screen 13) — and pushes. Proposals are generated only for coverage-universe names, only from non-sponsored segments, and only where the claim either disagrees with a current `data.json` value (company-sourced statements only) or is a first-appearance scoop, company guidance call, or very-big-deal claim worth a log line; every proposal carries its evidence (date, quote, speaker role, link to the moment) and a fingerprint so a rejected proposal is never raised twice. `proposals.json` holds the pending queue plus decisions until they are applied or 30 days old, then only fingerprints — it is a queue, not a history. `node proposals-apply.js` (run by the Action, never by hand) applies accepted proposals and refuses to write if the data checks would get worse. The source registry entry for each feed (§9) sets the provenance ceiling a fact from it can carry; a site change proposed from commentary is still `estimated` until a filing confirms it.
