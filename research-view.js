...
(function (root) {
  ...
  function render(data, params) {
    ...
    if (tab === 'developments') {
      ...
      const view = query.get('view') === 'earnings' ? 'earnings' : 'contracts';
      const asOf = outlook.asOf;
      const recorded = asOf ? 'Research as of ' + date(asOf) : 'Research date not supplied';
      const switcher = '<nav class="subnav" aria-label="Outlook views"><a href="' + esc(href({ view: 'contracts', limit: '' })) + '"' + (view === 'contracts' ? ' aria-current="page"' : '') + '>Contract outlook</a><a href="' + esc(href({ view: 'earnings', limit: '' })) + '"' + (view === 'earnings' ? ' aria-current="page"' : '') + '>Earnings outlook</a></nav>';
      ...
      content = '<div class="snapshot-banner"><span><strong>' + esc(recorded) + '.</strong> These are dated analyst judgements from the saved tracker. Probabilities and expectations are historical research, not current company guidance.</span></div>' + switcher + '<section class="panel" aria-labelledby="research-section-title"><div class="panel-header"><div><h2 id="research-section-title">' + (view === 'contracts' ? 'Contract outlook' : 'Earnings outlook') + '</h2><p>' + (view === 'contracts' ? 'Recorded probability of a binding revenue commitment within roughly six months of the research date.' : 'The questions and expectations recorded around each earnings event.') + '</p></div>' + filter + '</div>' + (shown.length ? shown.map(entry => '<article class="feed-item"><div class="feed-meta">' + companyLink(entry.tk) + '<span class="source-pill">Analyst judgement</span>' + (view === 'earnings' ? stamp(entry.date) : '') + '</div>' + (view === 'contracts' ? '<h3>' + (typeof entry.prob === 'number' ? esc(entry.prob) + '% recorded probability' : 'Probability not supplied') + '</h3><p>' + esc(entry.window || 'Window not supplied') + '</p><div class="section-caption">Expected capacity: ' + (typeof entry.mwLo === 'number' && typeof entry.mwHi === 'number' ? esc(entry.mwLo.toLocaleString('en-US') + '–' + entry.mwHi.toLocaleString('en-US')) + ' MW · source basis' : 'Not supplied') + '</div>' : '<p>' + esc(excerpt(entry.keyNumber || 'No research focus supplied.', 300)) + '</p>') + recordDetails(entry) + '</article>').join('') + more(rows.length, shown.length) : empty(view === 'contracts' ? 'contract forecasts' : 'earnings forecasts')) + '</section>';
    }
